					...
					for i,pos in ipairs(lsc.locations) do
						...
						if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
							if pos.locked == false then
								DisplayHelp("Appuyer sur ~INPUT_CONTEXT~ pour accèder à Los Santos Customs", false)
								...
								drawTxt("~r~Fermé, patientez s'il vous plait",1, 2, 0.5, 0.9, 0.6, 255, 255, 255, 255)
							end
						end
					end
				end
