        ...
        if IsControlJustPressed(1, 182) then
            ...
            if vehFront > 0 then
			    if lockstatus <= 1 then
                   ...
                   Notify("~r~Le coffre de cette voiture est verrouillé")	
                end				
            end
        end
        Menu.renderGUI()
...
function GetItem(arg)
    ...
    if vehFront > 0 then
        ...
        if (type(qty) ~= "number") then
            Notify("~r~Ceci n'est pas une quantité valide")
            return false
        end
        if tonumber(qty) <= tonumber(qtymax) and tonumber(qty) > -1  then
            ...
            Notify("~r~Vous n'avez pas autant de " .. lib .. " dans votre inventaire")
        end
    end
    Menu.hidden = true
end
...
function PutItem(arg)
    ...
    if vehFront > 0 then
        ...
        if tonumber(qty) <= tonumber(qtymax) and tonumber(qty) > -1 then
            ...
            Notify("~r~Il n'y a pas autant de " .. lib .. " dans le coffre")
        end
    end
    Menu.hidden = true
end
