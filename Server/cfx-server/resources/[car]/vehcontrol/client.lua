			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 174, true) -- INPUT_VEH_NEXT_RADIO
						DisableControlAction(0, 175, true) -- INPUT_VEH_PREV_RADIO
						...
						if not IsPauseMenuActive() then
							...
							if IsDisabledControlJustReleased(0, 174) then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								if cstate == ind_state_l then
									...
									actv_ind_timer = false
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TriggerEvent('InteractSound_CL:PlayOnOne', 'blank', 0)
								else
									state_indic[veh] = ind_state_l
									actv_ind_timer = true
									PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TriggerEvent('InteractSound_CL:PlayOnOne', 'indic', 0.1)
