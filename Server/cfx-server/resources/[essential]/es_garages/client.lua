...
local garages = {
	{name="Garage", colour=1, id=357, x=215.124, y=-791.377, z=29.646},
  	{name="Garage", colour=1, id=357, x=-334.685, y=289.773, z=84.705},
  	{name="Garage", colour=1, id=357, x=-55.272, y=-1838.71, z=25.442},
  	{name="Garage", colour=1, id=357, x=126.434, y=6610.04, z=30.750},
    {name="Garage", colour=1, id=357, x=-956.405, y=-2704.759, z=13.831},
    {name="Garage", colour=1, id=357, x=2241.75, y=5170.96, z=58.5396}
}
...
Citizen.CreateThread(function()
	while true do
		...
		for _, garage in pairs(garages) do
			DrawMarker(1, garage.x, garage.y, garage.z, 0, 0, 0, 0, 0, 0, 3.001, 3.0001, 0.5001, 255, 0, 0, 200, 0, 0, 0, 0)
			if GetDistanceBetweenCoords(garage.x, garage.y, garage.z, GetEntityCoords(LocalPed())) < 3 and IsPedInAnyVehicle(LocalPed(), true) == false then
				DisplayHelp("Appuyer sur ~INPUT_CONTEXT~ pour accèder au garage", false)
				--drawTxt("Appuyer sur ~g~E~s~ pour ouvrir le menu",1, 2, 0.5, 0.9, 0.6, 255, 255, 255, 255)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(vente_location[1],vente_location[2],vente_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			DisplayHelp("Appuyer sur ~INPUT_CONTEXT~ pour vendre le véhicule à 50% du prix d\'achat", false)
			--drawTxt("Appuyer sur ~g~E~s~ pour vendre le véhicule à 50% du prix d\'achat",1, 2, 0.5, 0.9, 0.6, 255, 255, 255, 255)
			if IsControlJustPressed(1, 86) then
...
AddEventHandler('garages:SpawnVehicle', function(vehicle, plate, state, primarycolor, secondarycolor, pearlescentcolor, wheelcolor)
    ...
    Citizen.CreateThread(function()
        ...
        if DoesEntityExist(caisseo) then
            ...
            if state == "Sorti" then
                drawNotification("~r~Ce véhicule n'est pas dans le garage")
            else
                local mods = {}
                for i = 0,24 do
                ...
                drawNotification("Le véhicule est ~g~sorti")
...
AddEventHandler('garages:StoreVehicleTrue', function()
	Citizen.CreateThread(function()
		...
		drawNotification("Le véhicule est ~g~rentré")
		TriggerServerEvent("garages:CheckGarageForVeh")
	end)
end)

AddEventHandler('garages:StoreVehicleFalse', function()
	drawNotification("~r~Ce n'est pas ton véhicule")
end)
