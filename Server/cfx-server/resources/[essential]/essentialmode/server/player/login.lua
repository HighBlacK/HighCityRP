...
function registerUser(identifier, source)
	hasAccount(identifier, function (result)
		if not result then
			...
			MySQL.Async.execute("INSERT INTO user_vehicle (`identifier`, `vehicle_name`, `vehicle_model`, `vehicle_price`, `vehicle_plate`, `vehicle_state`, `vehicle_colorprimary`, `vehicle_colorsecondary`, `vehicle_pearlescentcolor`, `vehicle_wheelcolor`) VALUES (@username, 'Panto', 'Panto', '4000',@plate, 'Rentré', '4', '0', '111', '156')",
			{['@username'] = identifier, ['@plate'] = string.upper(plateveh)})
			
			MySQL.Async.execute("INSERT INTO user_licence (identifier) VALUES (@identifier)", {['@identifier'] = identifier})
			MySQL.Async.execute("INSERT INTO alcooltaux (identifier) VALUES (@identifier)", {['@identifier'] = identifier})
