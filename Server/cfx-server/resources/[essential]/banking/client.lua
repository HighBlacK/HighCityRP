...
function ShowInfoUn(text, state)
	SetTextComponentFormat("STRING")
	AddTextComponentString(text)
    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
		  ShowInfoUn("Appuyer sur ~INPUT_CONTEXT~ pour accéder à la banque", 0)
        ...
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent("itinerance:notif",'~r~Vous ne pouvez pas utiliser la banque dans un véhicule')
			--DrawNotif('~r~Vous ne pouvez pas utiliser la banque dans un véhicule')
