    ...
    if (user) then
      ...
      MySQL.Async.fetchAll("SELECT * FROM user_vehicle WHERE identifier = @username",{['@username'] = player}, function (result)
        if (result) then
          ...
          if count == 10 then
            TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "~g~Votre garage est plein!\n")
          else
            if (tonumber(user.money) >= tonumber(price)) then
              user:removeMoney((price))
              TriggerClientEvent('FinishMoneyCheckForVeh', source, name, vehicle, price)
              TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Votre voiture est ~g~livrée. ~n~Bonne route!\n")
            else
              TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "~r~Fonds insuffisants!\n")
            end
          end
        else
          if (tonumber(user.money) >= tonumber(price)) then
