...
function VMenu.k_down()
	...
	local infinite = 0
	while (VMenu.items[VMenu.curMenu][VMenu.curItem].type == "separator" or VMenu.items[VMenu.curMenu][VMenu.curItem].type == "affiche" or VMenu.items[VMenu.curMenu][VMenu.curItem].type == "afficheA") do
		VMenu.curItem = VMenu.curItem + 1;
		if VMenu.curItem > #VMenu.items[VMenu.curMenu] then
			VMenu.curItem = 1
			infinite = infinite + 1
...
function VMenu.AddTxT(bindMenu, item)
	local size = #VMenu.items[bindMenu]
	VMenu.items[bindMenu][size+1] = {}
	VMenu.items[bindMenu][size+1].type = "affiche"
	VMenu.items[bindMenu][size+1].name = item
	VMenu.items[bindMenu][size+1].desc = nil
end

function VMenu.AddTxTA(bindMenu, item)
	local size = #VMenu.items[bindMenu]
	VMenu.items[bindMenu][size+1] = {}
	VMenu.items[bindMenu][size+1].type = "afficheA"
	VMenu.items[bindMenu][size+1].name = item
	VMenu.items[bindMenu][size+1].desc = nil
end
...
function Construct()
	...
	local menu = 5
	VMenu.AddMenu(menu, "", "policelocker") -- default = Header "Texte" sur fond bleu
	...
	local menu = 7
	VMenu.AddMenu(menu, "", "policegarage") -- default = Header "Texte" sur fond bleu
	...
	VMenu.AddTxT(menu,"")
	VMenu.AddTxT(menu,"Jobs réglementés (~b~Licence~s~)")
	VMenu.AddTxT(menu,"")
	for _, item in pairs(jobslicence) do
		VMenu.AddFunc(menu, item.name, "vmenu:poleemploi", {item.id}, "Valider")
	end


	local menu = 16
	VMenu.AddMenu(menu, "", "dirtymoney")
	VMenu.AddFunc(menu, "Laver votre argent sale (30% de retour)", "vmenu:cleanCash", {}, "Valider")

	local menu = 17
	VMenu.AddMenu(menu, "", "policeheli") -- default = Header "Texte" sur fond bleu
	...
	VMenu.AddFunc(menu, "Permis Vélo", "ply_autoecole:StartPermisVeloTrue", {}, "Gratuit")
	VMenu.AddFunc(menu, "Permis Poids lourd", "ply_autoecole:StartPermisPoidslourdTrue", {}, "Prix: 10000$")
	VMenu.AddFunc(menu, "Permis Hélicoptère", "ply_autoecole:StartPermisHeliTrue", {}, "Prix: 50000$")
	VMenu.AddFunc(menu, "Permis Bateau", "ply_autoecole:StartPermisBoatTrue", {}, "Prix: 50000$")
	...
	if User.jobs == 13 then
		...
		VMenu.AddSep(menu, "~r~Vous devez être en ambulancier")
	end
	...
	VMenu.ResetMenu(menu, "", "default")
	VMenu.AddFunc(menu, "Fermer", "vmenu:closeMenu", {}, "Fermer le menu")
	VMenu.AddFunc(menu, "Gestion des comptes publics", "publicfunds:gestion", {}, "Accéder")
	...
	local menu = 25
	VMenu.AddMenu(menu, "", "default")
	VMenu.AddFunc(menu, "Regulateur de vitesse", "vehmenu:vehOG", {}, "Accéder")
...
function getMainMenu()
	if VMenu.police == false and VMenu.telephone == false and VMenu.animations == false and VMenu.permis == false and VMenu.inventory == false and VMenu.medic == false then
		TriggerServerEvent('vmenu:updateUser', 98)
		TriggerServerEvent("inventory:getItems_s")
		VMenu.ResetMenu(98, "", "default")
		Wait(10)
		VMenu.AddTxT(98, User.prenom .. " " .. User.nom)
		VMenu.AddTxT(98, lang.menu.mainmenu.tel .. User.telephone)
		VMenu.AddTxT(98, jobsname[User.job])
		VMenu.AddTxT(98,"")
		...
		VMenu.AddFunc(98, "Donnez de l'argent", "vmenu:giveCash", {User.money}, lang.common.access)
...
function getGaragePolice()
	TriggerServerEvent('vmenu:updateUser', 7)
	Wait(200)
	VMenu.ResetMenu(7, "", "policegarage")
	if User.enService == 1 then
		if User.police >= 1 then
...
function getGarageHelicoAmbulance()
	TriggerServerEvent('vmenu:updateUser', 21)
	Wait(200)
	VMenu.ResetMenu(21, "", "default")
	if User.job == 13 then
		VMenu.AddFunc(21, "Helico d'ambulancier", "vmenu:getAmbulanceHelicoGarage", {"polmav"}, "Obtenir cet hélicoptère")
	else
		VMenu.AddSep(21, "Vous devez être en ambulancier")
	end
end
...
function getGarageHelicoPolice()
	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), false))then
	    VMenu.AddFunc(17, "Ranger l'hélicoptère", "jobspolice:despawnheli", {}, "Ranger l'hélicoptère")
    end
end

function getLockerPolice()
	VMenu.ResetMenu(5, "", "policelocker")
...
function getautoecole()
   VMenu.ResetMenu(19,"","autoecole")
   VMenu.AddMenu(19, "", "autoecole")
   VMenu.AddFunc(19, "Fermer", "vmenu:closeMenu", {}, "Fermer le menu")
   VMenu.AddFunc(19, "Permis Voiture", "ply_autoecole:StartPermisVoitureTrue", {}, "Gratuit")
   VMenu.AddFunc(19, "Permis Moto", "ply_autoecole:StartPermisMotoTrue", {}, "Prix: 5000$")
   VMenu.AddFunc(19, "Permis Vélo", "ply_autoecole:StartPermisVeloTrue", {}, "Gratuit")
   VMenu.AddFunc(19, "Permis Poids lourd", "ply_autoecole:StartPermisPoidslourdTrue", {}, "Prix: 10000$")
   VMenu.AddFunc(19, "Permis Hélicoptère", "ply_autoecole:StartPermisHeliTrue", {}, "Prix: 50000$")
   VMenu.AddFunc(19, "Permis Bateau", "ply_autoecole:StartPermisBoatTrue", {}, "Prix: 50000$")
end

function getpublicfunds()
   VMenu.ResetMenu(23,"","default")
   VMenu.AddMenu(23,"","default")
   VMenu.AddFunc(23, "Fermer", "vmenu:closeMenu", {}, "Fermer le menu")
   VMenu.AddFunc(23, "Gestion des comptes publics", "publicfunds:gestion", {}, "Accéder")
end
	...
	if VMenu.visible then
		...
		if VOpts.toUpdate ~= nil then -- Ne Pas Toucher
			...
			if VOpts.toUpdate == "itemBuy" then
				...
				for _, item in pairs(inv_array_legal) do
					VMenu.EditFunc(11, item.name, "inventory:buy", {getOpt("itemBuy"), item.id, item.price, item.name}, "Prix: " .. getOpt("itemBuy")*item.price .. "$")
				end
				for _, item in pairs(inv_array_boulangerie) do
					VMenu.EditFunc(20, item.name, "inventory:buy", {getOpt("itemBuy"), item.id, item.price, item.name}, "Prix: " .. getOpt("itemBuy")*item.price .. "$")
				end
				for _, item in pairs(inv_array_alcool) do
		...
		elseif (IsNearPoints(autoecole, 4) == true) then
		    VMenu.autoecole = true
		elseif (IsNearPoints(boulangerie, 4) == true) then
		    VMenu.boulangerie = true
		elseif (IsNearPoints(weashop, 4) == true) then
		    VMenu.weashop = true
		elseif (IsNearPoints(publicfunds, 2) == true) then
		    VMenu.publicfunds = true
		elseif (IsNearPoints(bars, 2) == true) then
		    VMenu.bars = true
		else
			if VMenu.curMenu ~= 98 then
				TriggerEvent("vmenu:toggleMenu")
        ...
        --if IsControlPressed(0, VMenu.adminkey) then
		  -- TriggerEvent(vmenu:openMenu, 0)
		--end
		...
		if IsControlPressed(0, VMenu.openKey) then
			...
			elseif (IsNearPoints(Tanker_company, 3) == true) then
				if User.job == 6 then
					...
					ShowMsgtime.msg = '~r~ Vous devez être cammioneur !'
			...
			elseif (IsNearPoints(informateur_coke, 3) == true) then
				TriggerEvent("vmenu:openMenu", 27)
			elseif (IsNearPoints(informateur_meth, 3) == true) then
		...
		if (IsNearPoints(LockerPolice, 4.5) == true and User.police >= 1) then
			...
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder aux casiers', false)
		elseif (IsNearPoints(Armory, 2) == true and User.police >= 1) then
			VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour accéder à l'armurerie", false)
		elseif (IsNearPoints(Garage_police, 5) == true and User.police >= 1) then
			VMenu.garagepolice = false
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder au garage', false)
		...
		elseif (IsNearPoints(Container_company, 4) == true and User.job == 6) then
			VMenu.ContainerCompany = false
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder à la compagnie de livraison', false)
		elseif (IsNearPoints(Frigorifique_company, 4) == true and User.job == 6) then
		...
		elseif (IsNearPoints(OutfitsShop, 4) == true) then
			...
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder à la boutique', false)
		elseif (IsNearPoints(BarberShop, 4) == true) then
			VMenu.barbershop = false
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder au coiffeur', false)
		elseif (IsNearPoints(Hospital, 4) == true) then
			VMenu.hospital = false
			VMenu.Info('Appuyer sur ~INPUT_CONTEXT~ pour accéder à l\'hopital', false)
		elseif (IsNearPoints(Store, 4) == true) then
			VMenu.store = false
			VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour accéder à l'épicerie", false)
		elseif (IsNearPoints(lavage_argent, 3) == true) then
		...
		elseif (IsNearPoints(boulangerie, 4) == true) then
			VMenu.boulangerie = false
		    VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour accèder à la boulangerie", false)
		elseif (IsNearPoints(weashop, 4) == true) then
		    VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour accèder à l'armurerie", false)
		elseif (IsNearPoints(publicfunds, 2) == true) then
		    VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour gérer les comptes publics", false)
		elseif (IsNearPoints(bars, 2) == true) then
		    VMenu.Info("Appuyer sur ~INPUT_CONTEXT~ pour accèder au bar", false)
		else
			if (VMenu.updatedChar == false) then
