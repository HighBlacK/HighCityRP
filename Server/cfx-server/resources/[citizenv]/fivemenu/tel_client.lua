...
AddEventHandler("menutel:PhoneOG", function(target, mytel)
	if not IsInVehicle() then
		...
		VMenu.AddFunc(98, "Applications", "menutel:applis", {target, mytel}, "Accéder")
...
RegisterNetEvent("menutel:applis")
AddEventHandler("menutel:applis", function(target, mytel)
    if not IsInVehicle() then
		TriggerEvent("vmenu:animChain", "cellphone@", "text_in", "cellphone@", "f_cellphone_text_read_base", "cellphone@", "cellphone_text_out")
		teldest = ""
		VMenu.telephone = true
		VMenu.ResetMenu(98, "", "default")
		Wait(100)
		VMenu.AddFunc(98, "Retour", "menutel:PhoneOG", {target, mytel}, "Retour")
		VMenu.AddSep(98,"~r~Aucune application installée sur cet appareil")
		VMenu.AddFunc(98, "Application Store", "menutel:appstore", {}, "En maintenance")
	else
		TriggerEvent("itinerance:notif", "~r~Utilisation impossible en conduisant")
	end
end)
