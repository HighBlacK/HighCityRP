...
AddEventHandler('poleemploi:jobs', function(id)
  TriggerEvent('es:getPlayerFromId', source, function(user)
    if (user) then
      ...
      TriggerClientEvent("citizenv:notify", source, "CHAR_AMANDA", 1, "Mairie", false, "Votre métier est maintenant : ~g~".. nameJob)
    else
  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    if (user) then
        ...
        TriggerClientEvent("jobslegal:getJobs", source, user:getJob())
	  	TriggerClientEvent("farm:getJobs", source, user:getJob())
		TriggerClientEvent("trucker:getJobs", source, user:getJob())
		TriggerClientEvent("fish:getJobs", source, user:getJob())
		TriggerClientEvent("delivery:getJobs", source, user:getJob())
    else
        TriggerEvent("es:desyncMsg")
    end
  end)
