...
LICENCES = {}
local prefecture_location = {237.869, -407.33, 47.9243}
...
function AcheterPermis()
    ...
    MenuTitle = "Permis"
    ClearMenu()
   --Menu.addButton("Permis de conduire(SOON)","OptionPermis",nil)
    Menu.addButton("Permis de port d'armes(SOON)","OptionPermis",nil)	
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('~r~Préfecture')
	...
	while true do
		...
		if GetDistanceBetweenCoords(prefecture_location[1],prefecture_location[2],prefecture_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			DisplayHelp('Appuyer sur ~INPUT_CONTEXT~ pour accèder à la préfecture', false)
